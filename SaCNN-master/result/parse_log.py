...
def parse_log(path_to_log):
    ...
    #regex_train_output = re.compile('iters., (\S+) = ([0-9]*\.?[0-9])')
...
def parse_avg_loss(log_path):
    f = open(log_path)
    log_lines = f.readlines()
    avg_loss = [float(line.split(' ')[-1]) for line in log_lines if ' solver.cpp:218] Iteration' in line]
    return avg_loss
